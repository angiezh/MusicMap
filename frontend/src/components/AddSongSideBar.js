...
const AddSongSideBar = ({ closeAddSongSidebar }) => {
  ...
  const addMoment = () => {
    console.log("Add Moment:", { selectedSongId, description });
    setDescription("");
  };
              ...
              <Row className="mx-2">
                {selectedSong ? (
                  <Col xs={12} className="mb-3">
                    <Card className="song-card" onClick={() => selectSong(null)}>
                      <div className="song-card__inner">
                        <div className="song-card__image">
                          <Card.Img src={selectedSong.image_url} className="img-fluid" />
                        </div>
                  ...
                  tracks.map((song, i) => (
                    <Col xs={12} md={6} lg={4} key={i} className="mb-3">
                      <Card className="song-card" onClick={() => selectSong(song.id)}>
                        <div className="song-card__inner">
                          <div className="song-card__image">
                            <Card.Img src={song.image_url} className="img-fluid" />
                          </div>
                          <div className="song-card__details">
                            <Card.Body>
